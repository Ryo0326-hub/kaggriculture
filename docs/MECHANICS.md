...
Cycle 11 verifies the UI’s eighteen unit actions in [the action reference](ACTIONS.md). Carrots start at one unit; bonus watering at ages two and three yields three without fertilizer or up to four with a timely application. Harvest removes the plant. Decay begins after the planned maturity day and removes one unit every two turns until it becomes a weed. The carrot tests check these mechanics independently from the policy. Its full-agent audit separately finds that urgent carrot watering bypasses the planned fertilizer sequence; engine correctness does not establish successful policy integration. [Results](CYCLE_11_RESULTS.md).
