...
Current checkpoint: **Cycle 14 packages the complete public V36 agent unchanged, at Ryo's request.** It is ready for a user-run Kaggle upload; server validation and rating are pending. Source hash `7eb5ab6c4858…`, complete attribution and provenance are retained. [Artifact and upload command](CYCLE_14_RESULTS.md). Cycle 3 remains protected and Cycle 12/13 artifacts are preserved. No local matches, automatic upload, cloud spending or training.

**Cycle 13's server loss changes the priority.** Episode 107984963 ends 66,319–106,333 and matches all 719 packaged decisions. No land candidate reaches valuation: the vacancy gate blocks 38 eligible calls and the order cap blocks the remaining call. The [postmortem](CYCLE_13_POSTMORTEM.md) documents why feature availability did not produce growth. The earlier sale-ordering experiment is superseded by establishing the requested unchanged public baseline. The [V36 review](V36_NOTEBOOK_REVIEW.md) remains the strategy reference, with its earlier recommendations retained as history. Inspect Cycle 14's actual expansion and execution before making improvements.
