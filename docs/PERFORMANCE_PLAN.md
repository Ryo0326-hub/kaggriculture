...
Current checkpoint: **Cycle 15 repairs the custom Cycle 13 agent at Ryo's request to fix all audit findings.** It is ready for a user-run upload; its own server validation and competitive evidence are pending. Source hash `ce4444126f5e…`. [Artifact, command and verification](CYCLE_15_RESULTS.md) · [CO/economics reasoning](CYCLE_15_OPTIMIZATION.md).

The six audited issues now have repairs: land admission and order-window blockers, terminal delivery precedence, fatal tomato watering, shared labor/reserve/hiring estimates, known fertilizer credits, and total-value investment selection. The recorded step-336 observation now yields a land-plus-eight-tomato order; this single-decision result is not an executed new season. Forty-five bounded tests, 54 isolated recorded decisions and fifteen fertilizer-transition comparisons pass. No local games were run.

Cycle 3 and all earlier frozen artifacts remain unchanged. **Cycle 14 still contains the complete public V36 agent byte-for-byte**, with independent provenance and pending server evidence. This repair does not merge legacy code into V36 or establish that the custom candidate is stronger. [Cycle 14](CYCLE_14_RESULTS.md) remains available for the originally requested public-baseline comparison.

The [Cycle 13 postmortem](CYCLE_13_POSTMORTEM.md) and [broader audit](CYCLE_13_LOGIC_AUDIT.md) describe the historical defective source. Its loss ended 66,319–106,333; all 719 decisions matched its artifact. Cycle 12's interrupted reference comparison remains incomplete; seeds 9401–9420 remain consumed evidence.

**Next evidence:** inspect Cycle 15's server validation and actual land/installation, wages, production, market fills and terminal delivery. Do not infer a rating or revenue gain from passing correctness checks. Preserve the public baseline and compare source-matched server behavior before further redesign.
