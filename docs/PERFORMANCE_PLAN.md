...
Cycle 18 incorporates all five reviews below. It uses carried feed/fertilizer only for the routes that can access it, stages profitable fertilizer for the next production day, reserves carried-stock space before purchases and keeps watering ongoing crops that still have output ahead. The same route choices now use faster incremental cost arithmetic. The second pre-submission audit repairs worker-specific service deadlines, missing-step handling and ambiguous partial animal deposits. The final `65e0e1f6f12e…` artifact passes 40 bounded tests and 140 fixed-observation checks, with no measured competitive result. [Audit](CYCLE_18_AUDIT.md). On the recorded Soumic Day 15 observation it rejects additional melon cohorts and admits two cows; these are forecasts, not realized counterfactual profits. Upload Cycle 18, then use its own returned games to check whether greater funded growth becomes delivered net income.
