...
def route_geometry(sites, access, return_to_shed=True):
        ...
        for members in combinations(range(len(sites)), count):
            best = min(
                (
                    ...
                    + (min(distance(sites[order[-1]], a) for a in access) if return_to_shed else 0),
                    order,
                )
...
def route_cover(sites, service_steps, access, capacity, dedicated=(), return_to_shed=True):
    ...
    if not sites:
        return (), (), True
    options = []
    handling = 2 if return_to_shed else 1
    for mask, order, travel in route_geometry(sites, access, return_to_shed):
        if len(order) > 1 and any(i in dedicated for i in order):
            continue
        duration = travel + handling + sum(service_steps[i] for i in order)
    ...
    if full not in dp:
        durations = tuple(
            travel + handling + service_steps[order[0]]
            for _, order, travel in route_geometry(sites, access, return_to_shed)
            if len(order) == 1
        )
...
def plan_turn(
    ...
    zones, durations, cover_feasible = route_cover(
        ...
        tuple(dedicated),
        return_to_shed=not overnight or terminal_day,
    )
...
def mixed_turn(
    ...
    expansion=0,
    overnight=False,
    dated_staffing=False,
    staffing_forecast=False,
    investments=True,
):
    ...
    staffing = None
    if dated_staffing and expansion:
        staffing = crop_route_staffing(obs, cfg, fields, plants, seeds, fertile, overnight)
        if staffing["feasible"]:
            worker_target = max(1, livestock_workers + len(staffing["routes"]))
    ...
    if expansion and day >= 2:
        market, report = expansion_investment(
            ...
            market,
            expansion,
            staffing=(overnight, dated_staffing) if staffing_forecast else None,
        )
...
def production_projection(
        ...
        for y, row in enumerate(farm["tiles"]):
            for x, tile in enumerate(row):
                if not isinstance(tile, dict):
                    continue
                if "animal" in tile:
                    animals[side].append({**tile, "site": (x, y)})
    ...
    for item in additions:
        ...
        if "animal" in item:
            ...
            column["site"] = item.get("site")
            column["new_planting"] = True
    ...
    private = obs["private"]
    occupied = {tuple(t["site"]) for t in animals[own] if t.get("site") is not None}
    free_sites = iter(p for p in farm_sites(obs)[0] if p not in occupied)
    ...
    for date in range(day, final + 1):
        ...
        staffing_detail = None
        if routed and staffing is not None:
            staffing_detail = projected_staffing(
                obs, cfg, date, animals[own], crops[own], *staffing
            )
            if not staffing_detail["feasible"]:
                return {"value": -1e9, "min_cash": -1e9, "daily": [], "route_feasible": False}
            workers = staffing_detail["workers"]
        elif routed:
            nodes = []
...
def farm_tours(nodes, access, capacity, return_to_shed=True):
    ...
    def duration(route):
        return (
            ...
            + (min(distance(route[-1], a) for a in access) if return_to_shed else 0)
            + sum(services[p] for p in route)
            + (3 if return_to_shed else 2)
        )
...
def crop_route_staffing(obs, cfg, fields, plants, seeds, fertile, overnight):
    """Reserve a crop crew for dated work, separately from committed animal routes.

    Open routes use ordinary-night deposit; closed routes protect final sales.
    This is a staffing bound, not a guarantee that greedy dispatch follows each tour.
    """
    day, hour = obs["day"], obs["hour"]
    tpd = cfg.get("turnsPerDay", 24)
    final = (cfg.get("episodeSteps", 720) - 2) // tpd
    ...
    for site, tile in plants:
        job = crop_job(tile, day, final)
        newborn = tile["planted_day"] == day and not tile["watered_today"]
        if not job and not newborn:
            continue
        work = 1
        if newborn or job[0] == "WATER":
            work += int(site in fertile)
            spec = CROPS[tile["crop"]]
            work += int(tile["yield_units"] > 0 and day - tile["planted_day"] >= spec["first"])
        nodes.append((site, work))
    pending = sum(seeds.get(c, 0) for c in CROPS)
    farm = obs["farms"][obs["player"]]
    empty = [
        p
        ...
        or (
            isinstance(farm["tiles"][p[1]][p[0]], dict)
            and farm["tiles"][p[1]][p[0]].get("kind") == "WEED"
        )
    ]
    for p in empty[:pending]:
        nodes.append((p, 2 + int(farm["tiles"][p[1]][p[0]] is not None)))
    routes, lengths, feasible = farm_tours(
        tuple(sorted(nodes)),
        shed_access(obs),
        max(1, min(tpd - 3, remaining - 2)),
        return_to_shed=not overnight or day == final,
    )
    return {
        "nodes": nodes,
        "routes": routes,
...
def projected_staffing(obs, cfg, date, animals, columns, overnight, dated):
    """Price the execution policy's separate animal/crop crews on one future date.

    Future crop service allows daily survival water plus dated fertilizer and harvest.
    This deliberately overestimates some optional water; future routes are estimates.
    Existing paid labor is a sunk cost, subtracted by production_projection.
    """
    tpd = cfg.get("turnsPerDay", 24)
    final = (cfg.get("episodeSteps", 720) - 2) // tpd
    access = shed_access(obs)
    active = [t for t in animals if t["placed_day"] <= date]
    crops = [c for c in columns if c["planted_day"] <= date <= c["end"]]
    if any(t.get("site") is None for t in [*active, *crops]):
        return {"workers": 13, "feasible": False}
    active.sort(key=lambda t: (distance(t["site"], access[0]), tuple(t["site"])))
    sites = tuple(tuple(t["site"]) for t in active)
    dedicated = []
    for i, tile in enumerate(active):
        spec = ANIMALS[tile["animal"]]
        age = date - tile["placed_day"] - spec["first_yield_day"]
        if age >= 0 and age % spec["interval"] == 0:
            dedicated.append(i)
    service = 2 if date == final else 3 if date == final - 1 else 4
    day_turns = min(tpd, cfg.get("episodeSteps", 720) - 1 - date * tpd)
    routes, _, feasible = route_cover(
        ...
        day_turns - 3,
        tuple(dedicated),
        return_to_shed=not overnight or date == final,
    )
    installing = any(t.get("pending") and t["placed_day"] == date for t in active)
    animal_workers = max(1, len(active) if installing or not feasible else len(routes))
    if not dated:
        # Match the legacy dispatcher's individual-trip hiring rule, including its cap.
        work = sum(2 * min(distance(c["site"], a) for a in access) + 3 for c in crops)
        workers = min(
            13,
            max(
                animal_workers + math.ceil(len(crops) / 4),
                math.ceil((6 * len(active) + work) / max(1, tpd - 4)),
            ),
        )
        return {
            "workers": workers,
            ...
            "feasible": True,
            "method": "legacy dispatch budget; no exact schedule certificate",
        }
    nodes = []
    farm = obs["farms"][obs["player"]]
    for col in crops:
        site = tuple(col["site"])
        tile = farm["tiles"][site[1]][site[0]]
        if col.get("new_planting") and col["planted_day"] == date:
            work = 3  # dig/plant/water allowance for a future vacant cell
        elif date == obs["day"] and isinstance(tile, dict) and tile.get("crop"):
            job = crop_job(tile, date, final)
            newborn = tile["planted_day"] == date and not tile["watered_today"]
            if not job and not newborn:
                continue
            work = 1
            if newborn or job[0] == "WATER":
                work += int(date in col["fertilizer"])
                work += int(
                    tile["yield_units"] > 0
                    and date - tile["planted_day"] >= CROPS[tile["crop"]]["first"]
                )
        elif date == final:
            work = int(date in col["outputs"])
        else:
            work = 1 + int(date in col["fertilizer"]) + int(date in col["outputs"])
        if work:
            nodes.append((site, work))
    crop_routes, _, feasible = farm_tours(
        tuple(sorted(nodes)),
        access,
        min(tpd - 3, day_turns - 2),
        return_to_shed=not overnight or date == final,
    )
    workers = animal_workers + len(crop_routes)
    return {
        "workers": workers,
        "animal_workers": animal_workers,
        "crop_workers": len(crop_routes),
        "feasible": feasible and workers <= 13,
        "method": "separate dated crop and livestock crews",
    }
...
def expansion_investment(obs, cfg, params, commands, market, max_land=3, staffing=None):
    ...
    if needed > held and len(market) < cfg.get("maxMarketOrdersPerTurn", 10):
        ...
        if missing and planned["farms"][obs["player"]]["money"] >= 150 + missing * 2 * price_at(
            ...
            planned = planning_snapshot(obs, cfg, commands, market, params)
    baseline = production_projection(planned, cfg, params, routed=True, staffing=staffing)
    report["baseline"] = baseline
    ...
    for land, orders, additions in options:
        ...
        projection = production_projection(
            planned, cfg, params, additions, routed=True, land_cost=land, staffing=staffing
        )
        cost = land + sum(
            ANIMALS[c["animal"]]["cost"] if "animal" in c else CROPS[c["crop"]]["seed"]
            for c in additions
