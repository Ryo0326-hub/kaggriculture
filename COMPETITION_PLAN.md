...
**Active plan:** the user approved an evidence-driven performance plan with local compute only and a protected incumbent. [Performance-first plan](docs/PERFORMANCE_PLAN.md) supersedes the staffing division, historical calendar and fixed Steps 9–11 below. [Server evidence and calibration](docs/SERVER_AND_BENCHMARK_CALIBRATION.md) records the first completed cycle.

Prepared September 9, 2026, revised September 10 after the Otter Vibe and SpaTaro replay analysis. **The current strategy and calendar are in [Revised competitive strategy](docs/REVISED_STRATEGY.md).** That document supersedes the original staffing assumptions, experimental priorities, and calendar below. Ryo leads development with approximately three hours per day; teammate help is optional to the critical path. Plan around 60 focused hours through September 29, plus September 30 as a buffer. At that planning checkpoint, Ryo reported $48.39 in AMD Developer Cloud credit expiring that day, with an unconfirmed expiry timezone. No droplet was assumed active. The sustained plan assumes the existing Mac and no out-of-pocket cloud spending.

The project now includes mixed production, coordinated livestock operations, dated investment models, conditional land expansion, evaluation, and isolated submission preparation; see [the status](README.md) and [Step 8 CO connections](docs/STEP_8_OPTIMIZATION.md). Steps 2, 3, 5, 6, and 7 have supplied server execution evidence. Step 7's [MugaBros loss and Jaikrishna win](docs/STEP_7_SERVER_ANALYSIS.md) show why more land must produce enough delivered output to cover its full cost. [Step 8 results](docs/STEP_8_RESULTS.md) compare conditional expansion with frozen Step 7 on the same fresh seeds and reactive opponents. Local promotion and Kaggle server validation remain separate. Preserve evaluation and release work if the schedule tightens.
...
Steps 5 and 6 implemented shared-worker livestock routes and bounded mixed production. [Step 7](docs/STEP_7_OPTIMIZATION.md) now adds joint opening portfolios, dated base/fertilized crop schedules, conditional two-stage rotations, and cash/inventory accounting. Its first validation attempt exposed late installation without time for feeding; the corrected policy preloads feed and protects the whole installation sequence. See [results and release status](docs/STEP_7_RESULTS.md).

Step 8 implements conditional land expansion with spatial staffing estimates, shared livestock routes, and nearby crop reservations. Its fresh pool includes independent expanding mixed/dairy variants. All 360 candidate games bought one additional quadrant; the implementation permits up to three owned quadrants, but the broader scale was not exercised in this validation. Forecast tours can underprice the actual dispatcher's crew, so realized results and operational checks remain necessary.

Step 9 will examine adaptive production and selling, including early harvest alternatives, explicit waiting, and a closer match between forecast and actual staffing costs. Step 10 covers selective maintenance and endgame economics; Step 11 covers final selection and release. Preserve the server-validated artifacts, write CO explanations, and use new validation seeds. The internal pool still does not include the actual reactive code of the stronger ladder rivals, and local results do not establish medal strength.
